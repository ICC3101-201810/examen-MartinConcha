...
namespace Examen
{
    public class Ghost
    {
        ...
        public Ghost()
        {
            GhostImages.Images.Add(Properties.Resources.pink_left1);
            GhostImages.Images.Add(Properties.Resources.red_left1);
        ...
        public void Set_Ghosts()
        {
            ...
            for (int y = 0; y < 30; y++)
            {
                for (int x = 0; x < 27; x++)
                {
                    if (Juego.gameboard.Matrix[y, x] == 15)
                    {
                        Amount++;
                        xStart[Amount] = x;
                        yStart[Amount] = y;
        ...
        private bool direction_ok(int x, int y, int ghost)
        {
            ...
            if (Juego.gameboard.Matrix[y, x] < 4 || Juego.gameboard.Matrix[y, x] > 10) { return true; } else { return false; }
        }
        ...
        {
            if (Juego.gameboard.Matrix[yCoordinate[ghost], xCoordinate[ghost]] < 4)
            {
        ...
        public void CheckForPacman()
        {
            for (int x = 0; x < GhostAmount; x++)
            {
                if (xCoordinate[x] == Juego.pacman1.xCoordinate && yCoordinate[x] == Juego.pacman1.yCoordinate)
                {
                    Juego.player.LoseLife(); break;
                }
            }
        }
    }
